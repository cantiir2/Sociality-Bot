...
const joox = (judul) => new Promise((resolve, reject) => {
    ...
    fetchJson(`https://api.lolhuman.xyz/api/jooxplay?apikey=${config.vhtear}&query=${judul}`)
        .then((result) => resolve(result))
        .catch((err) => {reject(err)
})
})

const spotify = (url) => new Promise((resolve, reject) => {
    console.log(`Getting Spotify music for ${url}...`)
    fetchJson(`https://api.lolhuman.xyz/api/spotify?apikey=${config.vhtear}&url=${url}`)
        .then((result) => resolve(result))
        .catch((err) => {reject(err)
})
})

const jooxv2 = (judul) => new Promise((resolve, reject) => {
    console.log(`Getting Joox music for ${judul}...`)
    fetchJson(`https://naufalhoster.xyz/dl/joox?apikey=${config.vhtear}&query=${judul}`)
        .then((result) => resolve(result))
        .catch((err) => {reject(err)
})
})
