...
const randomLewd = () => new Promise((resolve, reject) => {
    const tag = ['ecchi', 'lewdanimegirls', 'hentai', 'hentaifemdom', 'hentaiparadise', 'hentai4everyone', 'animearmpits', 'animefeets', 'animethighss', 'animebooty', 'biganimetiddies', 'animebellybutton', 'sideoppai', 'ahegao','flat']
 ...
 * @returns {Promise<object>}
 */
const flat = () => new Promise((resolve, reject) => {
    console.log('Searching for flat...')
    fetchJson('https://meme-api.herokuapp.com/gimme/flat')
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const loli = () => new Promise((resolve, reject) => {
    console.log(`Get Loli Pic!`)
    fetchJson(`https://api.vhtear.com/randomloli&apikey=${config.vhtear}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
